...
Syntax:
  python get_license_info_verbose.py -k <api_key> [-o <org_name>]
...
def getLicensesDetail(p_apiKey, p_organizationId):
    endpoint = "/organizations/%s/licenses" % p_organizationId
    success, errors, headers, response = merakiRequest(p_apiKey, "GET", endpoint, p_verbose=FLAG_REQUEST_VERBOSE)
    return success, errors, headers, response
...
def main(argv):
    ...
    for org in organizationList:
        print('\n---\n')
        success, errors, headers, licenses = getLicensesDetail(arg_apiKey, org["id"])
        pprint(licenses)
        orgId = org["id"]

        # Create local folder
        #todays_date = f'{datetime.now():%Y-%m-%d}'
        #folder_name = f'Org {orgId} {todays_date}'
        ...
        if not licenses is None:
            ...
            if "status" in licenses:
                pprint("%-20s%s" % ("Status:", licenses["status"]))

            if "expirationDate" in licenses:
                pprint("%-20s%s" % ("Expiration date:", licenses["expirationDate"]))

            if "licensedDeviceCounts" in licenses:
                pprint("\nLicensed device counts:")
                for deviceType in licenses["licensedDeviceCounts"]:
                    pprint("%-20s%s" % (deviceType, licenses["licensedDeviceCounts"][deviceType]))
        else:
            pprint('ERROR: Unable to fetch license info for organization "%s"' % org["name"])
